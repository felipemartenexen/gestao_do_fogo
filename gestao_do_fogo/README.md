...
Create a database named `gestao_do_fogo`.

```
createdb gestao_do_fogo
```
...
```bash
celery -A gestao_do_fogo worker -l INFO --pool=solo
```

Or with celery beat (for scheduled tasks):

```bash
celery -A gestao_do_fogo worker -l INFO -B --pool=solo
```
